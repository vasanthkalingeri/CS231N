...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    pad = (filter_size - 1)
    self.conv_param = {'stride': conv_stride, 'pad': pad / 2}
    
    # pass pool_param to the forward pass for the max-pooling layer
    self.pool_param = {'pool_height': pool_height, 'pool_width': pool_width, 'stride': pool_stride}
    
    
    w = (W + pad - filter_size) / conv_stride + 1
    h = (H + pad - filter_size) / conv_stride + 1
    
    wBar = (w - pool_width) / pool_stride + 1
    hBar = (h - pool_height) / pool_stride + 1
    
    self.params['W1'] = weight_scale * np.random.randn(num_filters, C, filter_size, filter_size)
    self.params['b1'] = np.zeros(num_filters)
    self.params['W2'] = weight_scale * np.random.randn(num_filters*wBar*hBar, hidden_dim)
  ...
  def loss(self, X, y=None):
    ...
    dl_dscores, dl_dW3, dl_db3 = affine_backward(dout, last_cache)
    dl_drelu, dl_dW2, dl_db2 = affine_relu_backward(dl_dscores, rel_cache)
    dl_dX, dl_dW1, dl_db1 = conv_relu_pool_backward(dl_drelu, conv_cache)
