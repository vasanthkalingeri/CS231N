...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    dl_dout1, dl_dW2, dl_db2 = affine_backward(dout, cache_l2)
    dl_dX, dl_dW1, dl_db1 = affine_relu_backward(dl_dout1, cache_l1)
    grads['b2'] = dl_db2
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    i = self.num_layers
    self.params['W' + str(i)] = weight_scale * np.random.randn(layer_dims[i - 1], num_classes)
  ...
  def loss(self, X, y=None):
    ...
    # layer, etc.                                                              #
    ############################################################################
    no_layers = self.num_layers #len(self.params) / 4
    ...
    for i in range(no_layers - 1):
        ...
        if self.use_batchnorm and self.use_dropout:
          out1, cache1 = affine_batchnorm_relu_dropout_forward(outputs[i],
          ...
          out1, cache1 = affine_batchnorm_relu_forward(outputs[i],
                                                        self.params['W' + str(i + 1)],
                                                        self.params['b' + str(i + 1)],
                                                        self.params['gamma' + str(i + 1)],
                                                        self.params['beta' + str(i + 1)], 
          ...
          out1, cache1 = affine_relu_dropout_forward(outputs[i],
                                                    self.params['W' + str(i + 1)],
                                                    self.params['b' + str(i + 1)],
                                                    self.dropout_param)
        else:
          out1, cache1 = affine_relu_forward(outputs[i],
                                            self.params['W' + str(i + 1)],
                                            self.params['b' + str(i + 1)])      
        outputs.append(out1)
        caches.append(cache1)
        
    scores, temp = affine_forward(outputs[-1], self.params['W' + str(no_layers)], self.params['b' + str(no_layers)])
    ...
    #Caculate the loss from the last layer
    dl_dout, dl_dW, dl_db = affine_backward(dout, caches[-1])
    grads['W' + str(no_layers)] = dl_dW + self.reg*self.params['W' + str(no_layers)]
    grads['b' + str(no_layers)] = dl_db
    
    #Backpropagae this loss
    dl_dprev = dl_dout
    for i in range(no_layers - 1, 0, -1):
        cache = caches[i-1]
        if self.use_batchnorm and self.use_dropout:
          dl_dout, dl_dW, dl_db, dgamma, dbeta = affine_batchnorm_relu_dropout_backward(dl_dprev, cache)
          
          grads['gamma' + str(i)] = dgamma
          grads['beta' + str(i)] = dbeta
        elif self.use_batchnorm:
          dl_dout, dl_dW, dl_db, dgamma, dbeta = affine_batchnorm_relu_backward(dl_dprev, cache)
          grads['gamma' + str(i)] = dgamma
          grads['beta' + str(i)] = dbeta
        elif self.use_dropout:
          dl_dout, dl_dW, dl_db = affine_relu_dropout_backward(dl_dprev, cache)
        else:
          dl_dout, dl_dW, dl_db = affine_relu_backward(dl_dprev, cache)
